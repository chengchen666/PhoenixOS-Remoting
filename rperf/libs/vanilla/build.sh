...
echo_cmd "cd build"
echo_cmd "cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DCMAKE_CUDA_ARCHITECTURES=$CUDA_ARCHITECTURE -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DHOOK_WITH_SAMPLE=$WITH_SAMPLE -DHOOK_VERBOSE_MAKEFILE=$VERBOSE_MAKEFILE -DCMAKE_INSTALL_PREFIX=$WORK_PATH/output -DCMAKE_SKIP_RPATH=ON .."
echo_cmd "make -j$(nproc --ignore=2)"
echo_cmd "make install"

pushd ..
rm -rf compile_commands.json && ln -s build/compile_commands.json compile_commands.json && chmod 666 compile_commands.json
popd
