...
int main() {
  ...
  // sanity check
  cudaMemcpyAsync(d_data, h_data, n, cudaMemcpyHostToDevice);
  char *h_data2 = (char *)calloc(n, sizeof(char));
  std::cout << (int)h_data2[0] << " " << (int)h_data2[1] << " "
            << (int)h_data2[2] << " " << (int)h_data2[3] << std::endl;
  cudaMemcpyAsync(h_data2, d_data, n, cudaMemcpyDeviceToHost);
  std::cout << (int)h_data2[0] << " " << (int)h_data2[1] << " "
            << (int)h_data2[2] << " " << (int)h_data2[3] << std::endl;
  for (int i = 0; i < n; i++) {
  ...
  // std::cout << "Average elapsed time: " << averageElapsedTime << " ms" <<
  // std::endl;
